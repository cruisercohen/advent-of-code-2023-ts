...
const parseline = (val) => {
    ...
    [...val].forEach((c) => { 
        if (!Number.isNaN(parseInt(c))) {
            ...
            lastDigit = parseInt(c);
        }
    });
    return firstDigit * 10 + lastDigit;
};

const parseLineWithText = (line) => {
    // ahh nedd to start at the end and work backwards, if we say replace '12oneight' we need to do the eight at the end first. of course if the one is the first number than we need to also replace that? hmmm
    const convertedVal = line.replaceAll('one', '1')
        .replaceAll('two', '2')
        .replaceAll('three', '3')
        .replaceAll('four', '4')
        .replaceAll('five', '5')
    ...
    return parseline(convertedVal);
}

const sum = () => {
    return calibrationValues.reduce( (acc, val) => {
        const num = parseline(val);
        return acc + num;
    }, 0)
}
...
const sumWithText = () => {
    return calibrationValues.reduce( (acc, val) => {
        const num = parseLineWithText(val);
        return acc + num;
    }, 0)
